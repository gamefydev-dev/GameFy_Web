...
export default function FormViewerPage() {
  ...
  // Normaliza options vindo do banco (text[] | string -> array<string>)
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // Suas RLS exigem authenticated. Se não houver user, joga pro login.
        const { data: authData } = await supabase.auth.getUser()
        const uid = authData?.user?.id

        if (!uid) {
          router.replace(`/login?next=/forms/${slug}`)
        ...
        if (form.cover_path) {
          ...
          if (!sErr && signed?.signedUrl) cover_url = signed.signedUrl
        }
        ...
        const msg = e?.message || 'Formulário indisponível.'
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      alert(e?.message || 'Falha ao enviar respostas.')
    }
  }
